import telebot, re, time
from random import randrange as rand 

send_pic = r".*?(пришли|скинь|отправь).*?(пикчу|фотку|фото).*?"
...
def response(bot, message):
    ...
    elif  "рикчай" in message.text.lower():
        bot.send_message(message.chat.id , "Пожалуйста, давай не будем говорить о таких страшных вещах 〜(＞＜)〜")

    elif  "спасибо" in message.text.lower():
        bot.send_message(message.chat.id , "Рада для тебя постараться :3")
